...
if __name__ == '__main__':
    ...
    df = df[((df["Count"] >= 1))]
    print(df.head())
    output_dir = "/data/Kaoutar/dev/py/kmers/filtered_kmers/cutoffs_iQR"
    ...
    cutoff_min =  Q1 - k * QR
    cutoff_max = Q3 + k * QR
    print(cutoff_min, cutoff_max)
    filtered_df = df[(df['Count'] >= cutoff_min) & (df['Count'] <= cutoff_max)] 
    output_file = "/data/Kaoutar/dev/py/kmers/filtered_kmers/cutoffs_iQR/filtered_iqr.csv"
