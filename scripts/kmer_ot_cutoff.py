...
class DistributionFitter:
    ...
    def compute_ot_loss(self, x_pair):
        ...
        return ot.emd2(self.empirical_histogram, model_pdf, self.cost_matrix, numItermax=100000000)

class CutoffOptimizer:
    def __init__(self, fitter, iterations=100000, tolerance=1e-6, verbose=True):
        self.fitter = fitter
    ...
    print("done")
    best_x1, best_x2 = map(int, np.round(sorted(result.x)))
    return best_x1, best_x2

def find_cutoffs(file_path, x1_init ,x2_init,iterations, tolerance=1e-6, verbose=True):
