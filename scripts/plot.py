...
def plot_kmer_distribution_aftercutoff(cutoff_name, df, cutoff_value, mean_value):
    ...
    sns.histplot(df["Count"], bins=400, color="blue", alpha=0.6)
...
data_paths = {
    "cut1_plus": "/data/Kaoutar/dev/py/kmers/filtered_kmers/filtered_cut1_plus.csv",
    "cut2_plus": "/data/Kaoutar/dev/py/kmers/filtered_kmers/filtered_cut2_plus.csv",
    "cut3_plus": "/data/Kaoutar/dev/py/kmers/filtered_kmers/filtered_cut3_plus.csv"
}

for name, cutoff_value in cutoffs.items():
    df = pd.read_csv(data_paths[name])  
    mean_value = df["Count"].mean() 
    #plt.savefig(f"{figures_dir}/kmer_distribution_{cutoff_value}.pdf") 
    plt.savefig(f"{figures_dir}/kmer_distribution_{cutoff_value}.pdf") 

if __name__ == '__main__':
    sequences, counts = read_fasta_file("/data/Kaoutar/dev/py/kmers/test_data/189N_lane1_20220816000_S16_L001_kmers_counts.txt", 5000)
    ...
    #df.to_csv("/data/Kaoutar/dev/py/kmers/filtered_kmers/kmers_5000.csv", index=False)
    #df = pd.read_csv("/data/Kaoutar/dev/py/kmers/filtered_kmers/kmers_5000.csv")
    lambda_poisson = df["Count"].mean() 
    std_poisson = np.sqrt(lambda_poisson)
    print("E", lambda_poisson, "STD", std_poisson)
    ...
    for i, cut in enumerate([1, 2, 3]):
        cutoffs[f"cut{cut}_plus"] = cutoffs_list[i][0]
        #cutoffs[f"cut{cut}_minus"] = cutoffs_list[i][1]
    print(cutoffs)

    """df= pd.read_csv("/data/Kaoutar/dev/py/kmers/filtered_kmers/caseplus/filtered_cut1_plus.csv")
    print(df.head())
    counts_column= "Count"
    max_value = df[counts_column].max()
    min_value = df[counts_column].min()
    ...
    for name, cutoff in cutoffs.items():
        ...
        """retained_percentage = len(filtered_df) / len(df) * 100
        retained_data.append({"Cutoff Name": name, "Cutoff Value": cutoff, "Retained %": retained_percentage})
    retained_df = pd.DataFrame(retained_data)
    retained_output_file = "/data/Kaoutar/dev/py/kmers/filtered_kmers/retained_percentages.csv"
    retained_df.to_csv(f"{output_dir}/retained_percentages.csv", index=False)"""

    #plot_kmer_distribution_cutoffs(df, cutoffs, save_path=f"{figures_dir}/kmer_distribution_cutoffs")
    #plot_kmer_distribution_kde(df, cutoffs, save_path=f"{figures_dir}/kmer_distribution_kde_cutoffs")

    plot_kmer_distribution_aftercutoff(name, df, cutoff_value, mean_value, figures_dir)
